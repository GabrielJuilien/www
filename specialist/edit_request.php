...
$request = $bdd->prepare('SELECT * FROM tasks WHERE ID_Specialist = ? AND ID_Request = ? AND Expedition_Datetime IS NULL');
...
$request = $bdd->prepare('SELECT
  ...
  JOIN requests ON requests.ID_Request = OUI.ID_Request
    ...
    <tr>
      ...
      <td class="filler">
        ...
        if (isset($data['Solve_Datetime'])) { ?>
          ...
          <button onclick="callbackDiscardChanges()">Discard changes</button><button onclick="callbackSaveChanges(<?php echo $data['ID_Request'] ?>)">Save changes</button><button onclick="callbackTransferRequest(<?php echo $data['ID_Request'] ?>)">Transfer request</button><button onclick="callbackSolveRequest(<?php echo $data['ID_Request'] ?>)">Solve request</button>
          <?php
        }
        ?>
        </div>
